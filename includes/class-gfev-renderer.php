...
/**
 * GFEV_Renderer Class
 * Handles all logic for rendering the entry viewer page or a custom login page.
...
class GFEV_Renderer {
    ...
    public function render_page() {
        ...
        if ( ! current_user_can( 'administrator' ) ) {
            wp_die( 'دسترسی غیرمجاز. این صفحه فقط برای مدیران سایت قابل مشاهده است.', 'خطای دسترسی', ['response' => 403] );
        }
    ...
    private function handle_login_submission() {
        if ( 'POST' === $_SERVER['REQUEST_METHOD'] && ! empty( $_POST['gfev_login_action'] ) ) {
            if ( ! isset( $_POST['gfev_login_nonce'] ) || ! wp_verify_nonce( $_POST['gfev_login_nonce'], 'gfev_login' ) ) {
                $this->login_error = 'خطای امنیتی. لطفاً صفحه را رفرش کرده و دوباره تلاش کنید.';
                return;
            }
    ...
    /**
     * Renders the custom login page HTML.
     */
        ...
        <div class="gfev-login-wrapper">
            <div class="gfev-login-card">
                ...
                <?php if ( ! empty( $this->login_error ) ) : ?>
                    <div class="gfev-login-error"><?php echo esc_html( $this->login_error ); ?></div>
                <?php endif; ?>

                <form name="loginform" id="loginform" action="" method="post">
                    <div class="gfev-form-group">
                    ...
                    <div class="gfev-form-options">
                        <label class="gfev-remember-me">
                            <input name="rememberme" type="checkbox" id="rememberme" value="forever">
                            <span>مرا به خاطر بسپار</span>
                        </label>
                    </div>
                    <div class="gfev-form-submit">
                        <input type="submit" name="wp-submit" id="wp-submit" class="gfev-button" value="ورود">
                        <input type="hidden" name="redirect_to" value="<?php echo esc_url( home_url( $_SERVER['REQUEST_URI'] ) ); ?>">
                        <input type="hidden" name="gfev_login_action" value="1">
                        <?php wp_nonce_field( 'gfev_login', 'gfev_login_nonce' ); ?>
                    </div>
                </form>
            </div>
        </div>
    ...
    /**
     * Renders the main entry viewer page for logged-in admins.
     */
                        ...
                        <div class="gfev-entry-card">
                            ...
                            <div class="gfev-card-body">
                                ...
                                foreach ( $form_object['fields'] as $field ) {
                                    ...
                                    echo '<div class="gfev-value-container">'; // A wrapper for values
                                    if ( $field->type === 'phone' ) {
                                    ...
                                    echo '</div>'; // End .gfev-value-container
                                    echo '</div>'; // End .gfev-field
                                }
                                ?>
                            </div>
                        </div>
    ...
    /**
     * Renders a phone number field with a WhatsApp link.
     * @param string $phone_number The raw phone number.
     */
    private function render_phone_value( $phone_number ) {
        ...
        if ( substr( $clean_number, 0, 1 ) === '0' ) {
            $clean_number = '98' . substr( $clean_number, 1 );
        }

        $whatsapp_url = 'https://wa.me/' . $clean_number;
        echo '<a href="' . esc_url( $whatsapp_url ) . '" class="gfev-whatsapp-link" target="_blank" rel="noopener noreferrer">' . $this->get_icon('whatsapp') . '<span>ارسال پیام</span></a>';
    }

    /**
     * Renders a single file item (image preview or file button).
    ...
    private function render_file_item( $url ) {
        ...
        if ( in_array( $ext, ['jpg', 'jpeg', 'png', 'gif', 'webp', 'svg'] ) ) {
            echo '<a href="' . esc_url( $url ) . '" target="_blank" rel="noopener noreferrer" class="gfev-image-preview-link"><img src="' . esc_url( $url ) . '" class="gfev-image-preview" alt="پیش‌نمایش تصویر" loading="lazy"/></a>';
        } else {
            echo '<a href="' . esc_url( $url ) . '" target="_blank" rel="noopener noreferrer" class="gfev-file-button">' . $this->get_icon('file') . '<span>دانلود فایل</span></a>';
        }
    }
